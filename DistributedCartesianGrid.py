...
class DistributedCartesianGrid(DistributedNode, CartesianGridBase):
    ...
    def processVisibility(self, task):
        ...
        gridPlaces = [(row, col), (row + 1, col), (row - 1, col), (row, col + 1), (row, col - 1)]
        zoneIds = []
        
        for gridRow, gridCol in gridPlaces:
            zoneIds.append(int(self.startingZone + ((gridRow * self.gridSize) + gridCol)))
        
        if (set(zoneIds) == set(self.visZones)):
            # Interest didn't change
        ...
        self.visZones = zoneIds
        messenger.send("avatarZoneChanged", [self.visAvatar, self.doId, zoneIds[0]])
        
        if not self.gridVisContext:
            self.gridVisContext = self.cr.addInterest(
                self.getDoId(), self.visZones,
                self.uniqueName("visibility"),
                event = self.uniqueName("visibility"))
        else:
            self.cr.alterInterest(
                self.gridVisContext, self.getDoId(), self.visZones,
                event = self.uniqueName("visibility"))
